...
| 2 | 판정 상수를 하나 건드리면 골든 테스트가 실패하고, 실패 메시지로 무엇이 어떻게 달라졌는지 실기 없이 판별할 수 있다 (TEST-01 후반부, ROADMAP 성공기준 1) | VERIFIED | red-trial 5차 실측(2026-08-28). `BleService.kt:2363` `fastContact` 게이트의 `warningStreak >= 2` → `>= 3` 1건 섭동 → EXIT=1, JUnit XML `tests="8" failures="2" errors="0" timestamp="2026-08-28T08:51:21"`, `32 actionable tasks: 32 executed`. 실패: `escalation_goldenTimeline`(:225), `release_goldenTimeline`(:248). 메시지 4요소 충족 — 테스트명 / `frame=22 stage=render` / 기대 `level=1 entry=2640 bcast=1` / 실제 `level=null entry=null bcast=0`. 원복 후 재실행 EXIT=0 `failures="0"`, `git status --porcelain -- app/` 공란(베이스라인 일치) |
...
**Score:** 4/4 truths verified (진실 2는 red-trial 5차 실측으로 해소 — Gaps Summary 절 참고, 진실 5(현장 관측)는 별도 사유로 human_verification 으로 라우팅)
...
|-------------|-------------|--------------|--------|----------|
| TEST-01 | 02-02-PLAN.md | 격상/해제 golden + 판정 상수 레드 트라이얼 | ✓ SATISFIED | 격상/해제 golden 자체 SATISFIED(진실 1). 레드 트라이얼(진실 2)도 5차 시도에서 검출 성공 — `:2363 warningStreak>=2` 섭동 시 8중 2실패, 4요소 메시지 충족, 원복 후 전수 복귀 |
...
ROADMAP Phase 2 성공기준 1은 두 절로 구성된다: (a) 격상/해제 전이 시점이 golden 과 일치, (b) 판정 상수를 건드리면 테스트가 실패한다. (a)는 `AlertCascadeGoldenTest.kt` 8개 테스트로 충족되었고, (b)는 초판 검증 시점에 미해소 gap 이었다 — 02-02 실행의 레드 트라이얼 4회가 전부 미검출(BUILD SUCCESSFUL, 8/8 pass)로 끝났고, 유일하게 유효할 것으로 특정된 후보는 사용자 승인 대기 상태로 남아 있었다.

**해소(2026-08-28, red-trial 5차 실측).** 승인 후 재개한 5차 시도에서 (b)가 실증되었다. 표적은 `BleService.kt:2363` — `fastContact` 게이트(Time-Gate 우회 발령 조건)의 `warningStreak >= 2`이며, 초판 문서가 적은 `:~2330`은 커밋 드리프트로 인한 오기여서 `grep -n` 으로 재특정했다. `>= 3` 1건 섭동 후 결과:

| 항목 | 섭동 | 원복 |
|---|---|---|
| EXIT | 1 | 0 |
| 빌드 | BUILD FAILED in 33s | BUILD SUCCESSFUL |
| JUnit XML | `tests="8" skipped="0" failures="2" errors="0"` @ `2026-08-28T08:51:21` | `failures="0" errors="0"` @ `2026-08-28T08:52:00` |
| 실행 확증 | `32 actionable tasks: 32 executed` | `32 actionable tasks: 32 executed` |

실패 테스트 2건: `escalation_goldenTimeline`(`:225`), `release_goldenTimeline`(`:248`). 실패 메시지가 4요소 규약을 전부 충족한다 — 어느 테스트(`escalation frame=22 stage=render`), 어느 프레임(`frame=022`), 기대값(`level=1 entry=2640 ... bcast=1`), 실제값(`level=null entry=null ... bcast=0`). 즉 실기 없이 실패 메시지만으로 "어떤 프레임의 무엇이 어떻게 달라졌는지" 판별 가능하다. 원복 후 재실행 EXIT=0, `git status --porcelain -- app/` 공란으로 베이스라인 복귀를 확인했다(프로덕션 커밋 0건).

부수적으로, 프로덕션 코드 리뷰(WR-01/WR-02)에서 확인된 두 가지는 이 gap 과는 별개로 BUG-02 수정 자체의 강건성에 관한 것이다 — 동시각 스캔 콜백 시 게이트가 우회될 수 있는 조건(WR-01)은 golden 시나리오의 고정 프레임 간격으로는 노출되지 않는다. truth-level FAIL 로 채점하지 않았으며(테스트된 시나리오 자체는 통과), 프로덕션 강건성 개선 항목으로 인계한다. 또한 `AlertCascadeGoldenTest.kt` KDoc 274-292행의 "레드 트라이얼 미검출" 기록은 5차 성공으로 낡았으므로 정정 대상이다.

---

*Verified: 2026-08-28 (재판정: gap 해소 후 verified)*
