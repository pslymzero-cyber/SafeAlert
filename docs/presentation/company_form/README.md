...
현장 현행 장비는 **I-PAS**(장비 본체 + 비콘 + 거치대)이고 1세트 약 54만원이다 (사용자 제공, 추정).

포지셔닝은 **우선 보완, 검증 후 판단**이다. 그래서 이 장은 대체 절감액을 주장하지 않는다.
주장하는 것은 하나다 — *I-PAS 가 덮지 않는 구간(보행자끼리 · EPJ · 미장착 장비)을 추가
하드웨어 없이 덮는다.* 확산 규모 환산(10대 540 · 30대 1,620 · 50대 2,700만원)은
**절감액이 아니라 같은 범위를 하드웨어로 덮을 때의 금액 규모**다. 슬라이드에 그대로 적었다.

세 가지 전제를 슬라이드 하단에 명시한다 — 단가가 추정치라는 것, 앱이 0원인 이유가
단말이 이미 지급돼 있기 때문이라는 것(미지급 인원은 BLE 태그 지급이 필요), 대체 여부는
이번 요청 범위 밖이라는 것. 이 세 줄이 없으면 안전 부서에서 동등성 주장으로 읽힌다.
