...
D.pill(s, 0.7, 4.62, "확산 환산", w=1.35)
for i, (n, won) in enumerate([("5개 센터", "1,655만원"), ("10개 센터", "3,310만원"), ("17개 센터", "5,627만원")]):
    x = 2.45 + i * 3.35
    D.text(s, x, 4.60, 3.2, 0.38, [(n + "   ", {"size": 12, "color": D.INK2}),
                                   (won, {"bold": True, "size": 18, "color": D.RED})])
for i, t in enumerate([
    "·  자동 셧다운은 SafeAlert 가 못 한다. I-PAS 를 걷어내자는 이야기가 아니다.",
    "·  단가는 사용자 제공 기준 (2026.08). 계약 조건에 따라 달라질 수 있다.",
    "·  지금은 개인 스마트폰에 임시 설치 중이다. PDA 로 옮기면 단말 비용 없이 업무 단말에서 돈다.",
    "·  대체 여부는 이번 요청 범위 밖 — 로드맵 완료와 현장 데이터 확보 후 재논의한다.",
]):
    D.text(s, 0.95, 5.26 + i * 0.31, 11.5, 0.30, [(t, {"size": 11.5, "color": D.INK2})])
D.quote(s, 6.52, "센터 17곳을 하드웨어로 덮으면 5,627만원. 앱은 그 범위를 추가 구매 없이 덮습니다.", x=0.7, w=8.6)
...
for i, t in enumerate([
    ...
    D.text(s, 0.95, 2.90 + i * 0.34, 11.5, 0.32, [(t, {"size": 11.5, "color": D.INK2})])
picture(s, "fig_radius.png", 4.85, 4.40, 3.6, "역할 조합별 판정 반경 (실제 비례 축척)", 648 / 358)
