...
CONTENT = [
    ...
    ("2. 기대효과", [
        ...
        (1, "  ③ 확산할수록 커지는 효과"),
        (2, "    → 단말이 늘면 상호 감지쌍이 배로 늘어남 (오른쪽 도식)"),
        (1, "  ④ 센터별 조건 대응"),
    ...
    ("3. 장단점", [
        ...
        (0, " "),
        (0, "2) 단점"),
        (1, "  ① 신호 세기로 거리를 추정해 환경에 흔들림"),
        (2, "    → 적재물 · 차체가 전파를 가리면 오차. 센터별 보정값으로 완화"),
    ...
    ("4. 사용자 피드백", [
        ...
        (1, "  ③ 3차 — 세이프존 도입 (오른쪽 도식)"),
        (2, "    → 휴게실 · 충전 구역에 비콘을 두면 그 구역에서 경보를 억제"),
        (0, " "),
        (0, "2) 현장에서 나온 요구"),
        (1, "  ① 보호가 끊긴 것을 알 수 없다 → 알림에 이상 표시"),
        (1, "  ② 멀어졌는데 다시 울린다 → 이탈 후 재발령 억제"),
        (1, "  ③ 알림 끄기가 번거롭다 → 본문 탭 = 즉시 무음"),
        (1, "  ④ 경보 화면이 눈에 안 들어온다 → 가장자리 부착 사이드바"),
    ]),
    ("5. 시연", [
        (0, "1) 동작 화면 · 설정 화면"),
...
# 본문 슬라이드 오른쪽에 붙일 삽화. render_figures.js 가 굽는다.
#   (파일명, 원본 폭, 원본 높이, 위쪽 위치, 캡션)
FIGURES = {
    "1. 개발 배경":   ("fig_aisle.jpg", 1200, 750, 1.55, "3m 렉 사이 통로 — 교차 구간에서 서로 보이지 않는다"),
    "2. 기대효과":    ("fig_pairs",   728, 278, 2.30, "단말 수가 늘면 상호 감지쌍은 더 빨리 늘어난다"),
    "3. 장단점":      ("fig_radius",  648, 358, 1.70, "역할 조합별 판정 반경 (실제 비례 축척)"),
    "4. 사용자 피드백": ("fig_zone",   668, 288, 2.15, "세이프존 — 구역 안에서는 양쪽 모두 조용하다"),
}
FIG_X, FIG_W = Inches(8.62), Inches(4.28)
...
for i, (title, lines) in enumerate(CONTENT):
    ...
    if fig:
        name, fw, fh, top, cap = fig
        h = Inches(FIG_W.inches * fh / fw)
        y = Inches(top)
        src = name if "." in name else name + ".png"
        s.shapes.add_picture(f"{HERE}/assets/{src}", FIG_X, y, width=FIG_W, height=h)
        tb = s.shapes.add_textbox(FIG_X, y + h + Inches(0.12), FIG_W, Inches(0.6))
        tb.text_frame.word_wrap = True
        para = tb.text_frame.paragraphs[0]
        para.alignment = PP_ALIGN.CENTER
        r = para.add_run(); r.text = cap
        r.font.size = Pt(10.5); r.font.color.rgb = RGBColor(0x5D, 0x6D, 0x7E)
...
r.text = "시연 영상 — 슬라이드쇼에서 재생 (경보음 포함)"
