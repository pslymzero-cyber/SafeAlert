...
D.table(s, 0.72, 1.58, 12.1,
        ...
        [["하드웨어", "차량 태그 52.5만 · 보행자 태그 11만 · EOD 22만", "없음 — 지급된 단말에 앱 설치"],
         ["설치 · 거치", "장비별 장착과 거치대 작업 필요", "없음"],
         ["대상 확대 시", "사람 · 장비가 늘면 태그를 그만큼 더 산다", "0 — 앱 설치만"],
         ["덮는 대상", "태그를 단 장비와 사람", "앱을 넣은 모든 단말 (보행자 · EPJ 포함)"],
...
D.text(s, 1.0, 4.74, 11.6, 0.36,
       [("일반 WF 센터 1곳 — PIT 4대 × 52.5만 + 상시 3명 × 11만 =  ", {"size": 13}),
        ("243만원", {"bold": True, "size": 16, "color": D.RED}),
        ("    (EOD 별도 · 인원 5명 센터는 265만원)", {"size": 12, "color": D.INK2})])

D.pill(s, 0.7, 5.42, "확산 환산", w=1.35)
for i, (n, won) in enumerate([("5개 센터", "1,215만원"), ("10개 센터", "2,430만원"), ("15개 센터", "3,645만원")]):
    x = 2.45 + i * 3.35
...
for i, t in enumerate([
    ...
    "·  대체 여부는 이번 요청 범위 밖 — 로드맵 완료와 현장 데이터 확보 후 재논의한다.",
]):
...
D.text(s, 0.95, 4.06, 11.9, 0.32,
       [("상시 인원 5명 센터 — 2,100,000 + 550,000 = ", {"size": 12}),
        ("2,650,000원", {"bold": True, "size": 12, "color": D.RED})])

D.pill(s, 0.7, 4.56, "확산 환산", w=1.35)
D.table(s, 0.72, 5.04, 12.1,
        ["센터 수", "5개", "10개", "15개 (FFWF 전체)"],
        [["같은 범위를 하드웨어로 덮을 때", "12,150,000원", "24,300,000원", "36,450,000원"]],
...
for i, t in enumerate([
    ...
    "·  EOD 는 수량 기준이 확인되지 않아 합계에서 뺐다. 포함하면 금액은 더 커진다.",
]):
    D.text(s, 0.95, 6.06 + i * 0.29, 11.0, 0.28, [(t, {"size": 11, "color": D.INK2})])
