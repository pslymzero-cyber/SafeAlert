...
FACTS = [
    ...
    ("추가 하드웨어 0", "센터 1곳 태그 비용 331만원 대비"),
...
# 단가·수량은 사용자 제공(2026.08). 산출 과정은 마지막 부록 장에 따로 뺐다.
s = prs.slides[5]
set_text(title_of(s), "3. 비용")
D.clear_body(body_of(s))
D.pill(s, 0.7, 1.12, "비교 기준", w=1.4)
D.text(s, 2.25, 1.15, 10.5, 0.3,
       [("I-PAS 는 3m 에서 장비를 세운다. SafeAlert 는 그 전에 사람에게 알린다 — 대체가 아니라 앞단이다.",
         {"size": 12, "color": D.INK2})])
D.table(s, 0.72, 1.58, 12.1,
        ["항목", "I-PAS (현행)", "SafeAlert"],
        [["하드웨어", "차량 태그 52.5만 + EOD 22만 (PIT 전량) · 보행자 태그 11만", "없음 — 지급된 단말에 앱 설치"],
         ["동작", "3m 내 감지 시 자동 셧다운 — 최후 개입", "15m 경고 · 8m 위험 경보. 장비 정지는 못 한다"],
         ["덮는 대상", "태그를 단 장비와 사람", "앱을 넣은 모든 단말 (보행자끼리 · EPJ 포함)"],
         ["대상 확대 시", "사람 · 장비가 늘면 태그를 그만큼 더 산다", "0 — 앱 설치만"],
         ["유지보수", "하드웨어 고장 · 배터리 교체", "앱 업데이트 (자동 배포)"]],
        col_w=[1.90, 5.30, 4.90], row_h=0.50, size=11.5)

box = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.72), Inches(4.58), Inches(12.1), Inches(0.66))
box.fill.solid(); box.fill.fore_color.rgb = RGBColor(0xF7, 0xF8, 0xF9)
box.line.color.rgb = D.LINE; box.line.width = Pt(1); box.shadow.inherit = False
box.text_frame.text = ""
D.text(s, 1.0, 4.74, 11.6, 0.36,
       [("일반 WF 센터 1곳 — PIT 4대 × (52.5만 + EOD 22만) + 상시 3명 × 11만 =  ", {"size": 13}),
        ("331만원", {"bold": True, "size": 16, "color": D.RED}),
        ("    (인원 5명 센터는 353만원)", {"size": 12, "color": D.INK2})])

D.pill(s, 0.7, 5.42, "확산 환산", w=1.35)
for i, (n, won) in enumerate([("5개 센터", "1,655만원"), ("10개 센터", "3,310만원"), ("15개 센터", "4,965만원")]):
    x = 2.45 + i * 3.35
    D.text(s, x, 5.40, 3.2, 0.38, [(n + "   ", {"size": 12, "color": D.INK2}),
                                   (won, {"bold": True, "size": 18, "color": D.RED})])
for i, t in enumerate([
    "·  자동 셧다운은 SafeAlert 가 못 한다. I-PAS 를 걷어내자는 이야기가 아니다.",
    "·  단가는 사용자 제공 기준. 절감액이 아니라 같은 범위를 하드웨어로 덮을 때의 금액이다.",
    "·  앱이 0원인 것은 단말이 이미 지급돼 있기 때문이다 (미지급 인원은 BLE 태그 필요).",
]):
    D.text(s, 0.95, 5.98 + i * 0.29, 8.7, 0.28, [(t, {"size": 11, "color": D.INK2})])
...
for i, t in enumerate([
    "①  경보만 한다 — 장비를 세우지 못한다. I-PAS EOD 의 3m 자동 셧다운은 대체 불가",
    "②  앱이 설치된 기기끼리만 감지 — 미설치 인원 · 장비는 보이지 않음",
    "③  기기가 밀집하면 화면 반응이 느려짐 (20대 이상). 안드로이드 8.0 이상 전용",
]):
...
# ── 13. 부록 — 산출 근거 ────────────────────────────────────
# CouSolve 자료의 08 부록(산출근거) 방식. 금액을 주장하는 장과 계산을 분리한다.
s = prs.slides[12]
set_text(title_of(s), "부록 — 산출 근거")
D.clear_body(body_of(s))
D.pill(s, 0.7, 1.12, "센터 1곳 기준", w=1.75)
D.text(s, 2.6, 1.15, 10.2, 0.3,
       [("일반 WF 센터 · PIT 장비 4대 · 상시 근무 3명 기준", {"size": 12, "color": D.INK2})])
D.table(s, 0.72, 1.60, 12.1,
        ["항목", "단가", "수량", "금액"],
        [["차량 태그", "525,000원", "PIT 4대", "2,100,000원"],
         ...
         ["합계", "", "", "3,310,000원"]],
        col_w=[3.30, 2.80, 3.00, 3.00], row_h=0.50, size=11.5,
        aligns=[PP_ALIGN.LEFT, PP_ALIGN.RIGHT, PP_ALIGN.CENTER, PP_ALIGN.RIGHT])
D.text(s, 0.95, 4.06, 11.9, 0.32,
       [("상시 인원 5명 센터 — 2,100,000 + 880,000 + 550,000 = ", {"size": 12}),
        ("3,530,000원", {"bold": True, "size": 12, "color": D.RED})])

D.pill(s, 0.7, 4.56, "확산 환산", w=1.35)
D.table(s, 0.72, 5.04, 12.1,
        ["센터 수", "5개", "10개", "15개 (FFWF 전체)"],
        [["같은 범위를 하드웨어로 덮을 때", "16,550,000원", "33,100,000원", "49,650,000원"]],
        col_w=[4.30, 2.60, 2.60, 2.60], row_h=0.50, size=11.5,
        aligns=[PP_ALIGN.LEFT, PP_ALIGN.RIGHT, PP_ALIGN.RIGHT, PP_ALIGN.RIGHT])
for i, t in enumerate([
    "·  단가는 사용자 제공 (2026.08 기준). 계약 조건에 따라 달라질 수 있다.",
    "·  SafeAlert 는 같은 대상(PIT 4대 + 상시 인원)을 이미 지급된 단말에 앱을 넣어 덮는다. 추가 하드웨어 0.",
    "·  다만 EOD 의 3m 자동 셧다운은 SafeAlert 가 대신하지 못한다 — 금액 비교는 감지 · 경보 범위에 한한다.",
]):
    D.text(s, 0.95, 6.06 + i * 0.29, 8.8, 0.28, [(t, {"size": 10.5, "color": D.INK2})])
D.quote(s, 6.92, "실제 절감액이 아니라, 같은 범위를 하드웨어로 덮을 때의 금액 규모입니다.", x=0.7, w=8.4)
