...
C.fill(C.body_of(s, "Solution"),
       [[("이미 나눠준 스마트폰에 앱을 깔면, 폰끼리 신호를 주고받아 서로를 감지한다.",
        ...
        [("정밀 측위(UWB)를 지원하는 폰끼리는 실제 거리로 — 지게차가 끼면 경고 15m · 위험 8m, "
          "사람끼리는 5m · 3m.", {"size": 10})],
        [("그 밖의 폰은 신호 세기로 판단한다. 창고는 랙과 적재물 때문에 신호 세기가 거리와 "
          "일정하게 대응하지 않아, 거리로 환산하지 않는다.", {"size": 10, "color": C.INK2})],
...
pic(s, "fig_radius.png", 1.05, 2.98, 3.80, 648 / 358,
    "정밀 측위 지원 폰끼리의 경보 거리 — 실제 비례로 그린 것")
...
s = S[6]
TRADE = [
    (2.37, [[("자주 울리기  vs.  꺼버리기", {"bold": True, "size": 12})],
            [("넓게 울리면 작업자가 앱을 꺼버린다. 꺼진 폰은 아무것도 못 한다.",
              {"size": 10, "color": C.INK2})]],
     [[("자주 울리되, 울릴 곳을 좁혔다", {"bold": True, "size": 12, "color": C.NAVY})],
      [("같은 상대와 같은 등급이 5초 넘게 이어지면 소리를 끈다 (v1.1.61) · "
        "휴게실 · 충전 구역은 안전 구역으로 지정 (v1.1.62) · "
        "멀어진 뒤 다시 울리지 않게 (v1.1.51)", {"size": 9.5, "color": C.INK2})]]),
    (3.96, [[("알리기만 하기  vs.  장비를 세우기", {"bold": True, "size": 12})],
            [("장비를 자동으로 세우려면 지게차마다 제동 장치를 달아야 한다. 앱은 못 한다.",
              {"size": 10, "color": C.INK2})]],
     [[("알리는 것부터 시작했다", {"bold": True, "size": 12, "color": C.RED})],
      [("안전 대책은 ‘자동으로 멈추기’ 가 ‘사람에게 알리기’ 보다 확실하다. 그건 맞다. "
        "다만 지금 우리 센터에는 둘 다 없다. 장비를 사지 않고 지금 당장 할 수 있는 쪽부터 "
        "넣었고, 자동 정지는 필요하다고 판단되면 별도 과제로 간다.",
        {"size": 9.5, "color": C.INK2})]]),
    (5.56, [[("빨리 고치기  vs.  같은 결과 내기", {"bold": True, "size": 12})],
            [(f"3개월에 {RELEASES}번 고쳤지만, 고쳤던 문제가 다시 나타났다.",
              {"size": 10, "color": C.INK2})]],
     [[("속도를 늦추고, 같은 결과를 택했다", {"bold": True, "size": 12, "color": C.NAVY})],
      [("고친 문제마다 자동 검사를 붙여 둔다. 지금 51건이 릴리스마다 돌고, 하나라도 "
        "실패하면 새 버전이 아예 나가지 않는다.", {"size": 9.5, "color": C.INK2})]]),
]
...
C.fill(C.body_of(s, "대상"),
       [[("지게차 ↔ 보행자 접근 경보 장비는 시중에 이미 여러 제품이 있고, "
        ...
        [("감지 거리 10~25m 는 이 앱이 정밀 측위(UWB)를 쓸 때의 경고 거리 15m 와 같은 범위다.",
          {"size": 10, "color": C.INK2}),
         ("   (제품 정보 — 벤더 공개 자료 2026.09 조회, 벤더 주장 효과 수치는 인용하지 않음)",
...
C.table(s, BX + 0.24, BY + 0.42, BW - 0.48,
        ...
        [["ZoneSafe", "RFID", "차량 주위 360° 최대 10m", "태그 필요", "없음 (알림만)", "공개 단가 없음"],
         ...
         [[[("SafeAlert  (이 제안)", {"bold": True, "color": C.TEAL})]], "BLE + UWB",
          "UWB 폰끼리 15m / 8m · 그 외 신호 세기",
          [[("없음 — 지급된 폰", {"bold": True, "color": C.TEAL})]], "없음 (알림만)",
          [[("0원", {"bold": True, "color": C.TEAL})]]],
        ],
        col_w=[2.30, 1.05, 3.85, 1.40, 1.15, 1.57], row_h=0.275, head_h=0.28,
...
C.fill(C.body_of(s, "Short term"),
       [[("업무용 스마트폰에 앱을 깔아, 폰끼리 서로를 감지하게 한다",
        ...
        [("·  정밀 측위(UWB) 폰끼리는 실제 거리로 — 지게차가 끼면 15m · 8m, 사람끼리 5m · 3m", {})],
        [("·  그 밖의 폰은 신호 세기로 판단한다 (창고에서는 거리로 환산하지 않는다)", {})],
        [("·  소리 · 진동 · 화면으로 동시에 알린다 — 하나가 막혀도 나머지가 전달된다", {})],
        [("·  상대가 지게차인지 사람인지, 후진 중인지까지 함께 보낸다", {})],
        [("·  안전 구역과 자동 소리 끄기로, 울릴 필요 없는 곳에서는 울리지 않는다", {})],
        [(f"아무 감지 수단도 없던 현장에서 시작해, 지금 {LATEST} 이 3개 센터에서 돌고 있다.",
          {"bold": True, "size": 10.5, "color": C.TEAL})]],
       size=10.5, margin=0.22, space=1.32, gap=4, top=C.TOPCHIP)
C.fill(C.body_of(s, "Long term"),
       [[("결과가 흔들리지 않게 다듬는 작업 — 5단계 중 4단계 완료",
          {"bold": True, "size": 12, "color": C.NAVY})],
        [("·  1 · 2단계 완료 — 자동 검사 체계를 붙이고, 안전과 직결된 동작을 고정했다", {})],
        [("·  3단계 완료 — 한 파일에 몰려 있던 경보 계산 코드를 나눴다 (3,674줄 → 1,971줄)", {})],
        [("·  4단계 완료 — 기기 상태 관리 경로를 하나로 통일했다", {})],
        [("·  5단계 남음 — 경보 계산을 화면과 분리해, 폰이 많아도 화면이 끊기지 않게 한다", {})],
        [("자동 검사 51건이 릴리스마다 돌고 있다.",
          {"bold": True, "size": 10.5, "color": C.TEAL})],
        [("그중 24건은 경보 동작을 프레임 단위로 고정한 검사다 — 코드를 나눠도 동작이 "
          "달라지지 않았음을 이 검사로 확인했다.", {"size": 10, "color": C.INK2})]],
       size=10.5, margin=0.22, space=1.32, gap=4, top=C.TOPCHIP)
...
t = [sh.table for sh in s.shapes if sh.has_table][0]
PLAN = [
    ("1 · 2단계  자동 검사 체계 · 안전 동작 고정 (저속 접근 포함)", "제안자", "완료", "완료"),
    ("3단계  경보 계산 코드 분리 (3,674줄 → 1,971줄)", "제안자", "완료", "완료"),
...
C.fill(C.at(s, 0.78, 2.37), [[("", {})]], margin=0.02)
C.text(s, 1.00, 2.56, 11.4, 0.26,
       [[("개선 계획  1 ~ 5단계  —  4단계까지 완료", {"bold": True, "size": 13, "color": C.NAVY}),
         ("     새 기능을 붙이는 게 아니라, 결과가 흔들리는 원인을 걷어내는 작업",
          {"size": 10.5, "color": C.INK2})]])
C.table(s, 1.00, 2.98, 11.35,
        ["단계", "무엇을 하는가", "출하 시 현장에서 확인할 것", "상태"],
        [["1", "고친 문제마다 자동 검사를 붙인다", "설치와 경보가 이전과 똑같이 되는가",
          [[("완료", {"bold": True, "color": C.TEAL})]]],
         ["2", "안전 동작을 자동 검사로 고정한다", "천천히 다가오는 지게차에 경고가 뜨는가",
          [[("완료", {"bold": True, "color": C.TEAL})]]],
         ["3", "한 파일에 몰린 경보 계산 코드를 나눈다", "나누기 전과 동작이 똑같은가",
          [[("완료", {"bold": True, "color": C.TEAL})]]],
         ["4", "기기 상태 관리 경로를 하나로 통일한다", "2시간 넘게 켜 둬도 느려지지 않는가",
          [[("완료", {"bold": True, "color": C.TEAL})]]],
         ["5", "경보 계산을 화면과 분리해 따로 돌린다", "폰 20대 이상에서 화면이 끊기지 않는가",
          [[("남음", {"color": C.INK2})]]]],
...
C.lines(s, 1.10, 5.50, 11.2, [
    ...
    [("③  어느 단계에서 멈춰도 쓸 수 있다", {"bold": True}),
     ("  —  단계마다 현장에 그대로 배포했고, 지금 버전이 계속 돌고 있다. 개선이 멈춰도 "
      "경보는 끊기지 않는다.", {"color": C.INK2})],
    [("현재  ", {"bold": True, "color": C.NAVY}),
     ("자동 검사 51건이 릴리스마다 돈다. 그중 24건은 경보 동작을 프레임 단위로 고정한 검사로, "
      "3 · 4단계에서 코드를 나눌 때 동작이 1비트도 달라지지 않았음을 이 검사로 확인했다.",
      {})],
], size=10, gap=0.32)
