...
- **EPJ**: 앱이 `card_role_epj` 를 `visibility="gone"` 으로 숨겨 두고 판정 로직만 살려 둔 상태다.
  자료에서도 '고르는 대상'(역할 선택, 시뮬레이터, 반경 조합 표기)에서는 빼고,
  현장 조건 설명과 1바이트 프로토콜 정의(`CAT 01`)에는 `UI 비노출` 표기와 함께 남긴다
- **설정**: `activity_ble_settings.xml` 의 실제 항목 구성을 따른다 —
