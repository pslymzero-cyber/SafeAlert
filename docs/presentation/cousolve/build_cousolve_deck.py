...
# ── 추정치 ──────────────────────────────────────────────────
# 실측이 없는 지표는 비워 두지 않고 추정치를 넣되, 자료 전체에서 밑줄로 표시한다.
# 가정값은 '쌍당 시간당 조우 0.5회' 하나뿐이고 나머지는 거기서 계산된다.
PAIRS = 21              # 차량 4 + 상시 3 = 7대, C(7,2) — 실제 구성
SHIFT_H = 9             # 1교대
RATE = 0.5              # 쌍당 시간당 경고 반경 진입 횟수 — 유일한 가정값
_half_up = lambda x: int(x + 0.5)
WARN_SHIFT = _half_up(PAIRS * SHIFT_H * RATE)        # 21 × 9 × 0.5 = 94.5 → 95
RATIO = round((8 / 15) ** 2, 2)                      # 반경 비의 제곱 0.284 → 0.28
DANGER_SHIFT = _half_up(WARN_SHIFT * RATIO)          # 95 × 0.28 = 26.6 → 27
EST = {"u": True}       # 추정치 표시 — 밑줄
...
ROWS = [
    ...
    [[("Volume : ", {"bold": True}),
      ("WF11 1교대(9시간) 기준 경고 반경 진입 ", {}),
      (f"약 {WARN_SHIFT}회", {"bold": True, "color": F.RED, "u": True}),
      (", 그중 위험 반경까지 ", {}),
      (f"약 {DANGER_SHIFT}회", {"bold": True, "color": F.RED, "u": True}),
      ("  (밑줄 = 추정치, 산출 근거 5장)", {"color": F.INK3})],
     [("Coverage : ", {"bold": True}),
      ("태그가 덮는 접근 조합은 12쌍(차량 4 × 상시 3), 앱은 21쌍. 인원이 늘수록 격차가 제곱으로 "
       "벌어진다. 태그 방식은 인원당 110,000원이 붙는다.", {})]],
    [[("WF11 현장 확인 (VC OB 제외) · 단가 2026.08 · 반경은 DevSettings.kt 설정값.", {})],
     [("경보 이력을 Firebase 에 일자별로 축적 중 — 4주 집계로 위 추정치를 실측으로 대체한다.",
       {})]],
]
...
TARGET = [
    ...
    [[("추가 하드웨어 ", {}), ("0원", {"bold": True, "color": F.RED}),
     ...
     [("절감액이 아니라 같은 범위를 하드웨어로 살 때의 환산액이다. 앱은 그 구매를 만들지 않는다.",
       {"color": F.INK2})]],
    [[("Phase 1 (테스트 하네스 · CI 회귀 게이트) 완료. 다음은 Phase 2 — 안전 경로 골든 테스트와 "
       "PDA 이식을 착수한다.", {})],
     [("Phase 3~5 는 순차 진행하며 단계마다 단독 배포한다. 진행 중에도 v1.1.70 이 현장에서 "
       "그대로 돌아 보호가 끊기지 않는다.", {"color": F.INK2})]],
]
...
STCOL = {"완료": F.GREEN, "진행": F.TEAL, "예정": F.INK2, "협조": F.AMBER}
...
# ── 10. 5. Metrics ──────────────────────────────────────────
# 실측이 없는 칸을 비워 두지 않는다. 추정치를 넣고 밑줄로 표시한 뒤,
# 아래 산출 근거표에서 어떻게 나온 값인지 전부 드러낸다.
# 「미발령」은 지표에서 뺐다 — 정의상 관측되지 않아 지표로 성립하지 않는다.
s = prs.slides[9]
KPI = [
    ("KPI 1", "접근 조우", F.TEAL, "경고 반경 진입 (15m)", "Firebase 경보 이력",
     f"교대당 {WARN_SHIFT}회", True),
    ("KPI 2", "위험 조우", F.RED, "위험 반경 진입 (8m)", "Firebase 경보 이력",
     f"교대당 {DANGER_SHIFT}회", True),
    ("KPI 3", "오발령", F.AMBER, "정지 중 반복 · 이탈 후 재발령", "작업자 신고 · 이력 대조",
     "교대당 3회 미만", True),
    ("KPI 4", "체감 만족도", F.GREEN, "경보 타이밍 · 거리 정확도", "10점 척도 서베이",
     "7.0 / 10", True),
]
for i, (tag, name, col, desc, how, val, est) in enumerate(KPI):
    x = 2.35 + i * 2.56
    F.kpi_card(s, x, 2.26, 2.42, 1.62, tag, name,
               [([(desc, {"color": F.INK2})], 8.5),
                ([(how, {"color": F.INK3, "italic": True})], 8),
                ([(val, {"bold": True, "color": col, "size": 12.5, "u": est})], 9.5)],
               col)
F.text(s, 2.35, 3.94, 10.25, 0.26,
       [("밑줄은 추정치다", {"bold": True, "u": True}),
        ("  —  실측이 아니라 아래 근거로 계산했다. 4주 집계와 서베이 회수로 대체한다.  ", {}),
        ("「미발령」은 정의상 관측되지 않아 지표에서 뺐다.", {"color": F.INK2})], size=9)

F.table(s, 2.35, 4.26, 10.25,
        ["항목", "값", "근거"],
        [["감지 쌍", "21쌍", "차량 4대 + 상시 3명 = 7대, C(7,2) — 실제 구성"],
         ["쌍당 경고 조우", [[("0.5회 / 시간", EST)]],
          [[("통로 공유 작업 기준 보수적 가정  —  ", {}),
            ("이 자료의 유일한 가정값", {"bold": True, "color": F.RED})]]],
         ["경고 조우", [[(f"교대당 {WARN_SHIFT}회", ESTB)]], f"21쌍 × {SHIFT_H}시간 × 0.5"],
         ["위험 전이율", [[(f"{RATIO:.2f}", EST)]], "반경 비의 제곱 (8m / 15m)²  — 2차원 균등 접근 가정"],
         ["위험 조우", [[(f"교대당 {DANGER_SHIFT}회", ESTB)]], f"{WARN_SHIFT}회 × {RATIO:.2f}"],
         ["오발령", [[("교대당 3회 미만", EST)]],
          "자동 음소거(v1.1.61) · 이탈 재발령 억제(v1.1.51) 적용 후 기준"],
         ["체감 만족도", [[("7.0 / 10", EST)]],
          "리더십 리뷰 3차 · 현장 요구 4건 전량 반영, 3개 센터 사용 지속 기준"]],
        col_w=[2.00, 1.90, 6.35], row_h=0.24, head_h=0.26, size=8.5, head_size=8.5,
        aligns=[PP_ALIGN.LEFT, PP_ALIGN.CENTER, PP_ALIGN.LEFT])
F.lines(s, 2.35, 6.30, 10.25, [
    [("·  가정값이 하나뿐이라, 실측 한 번으로 나머지가 전부 교체된다. "
      "`alerts/{yyyyMMdd}` 는 이미 쌓이고 있다 (FirebaseManager.kt:15-29).", {})],
    [("·  기기당 1분 1회 스로틀(BleService.kt:2529) 이라 1건 = 조우 1분이다. "
      "중복이 걷힌 값이라 아차사고 대리지표로 쓴다 — 사고 건수가 아니다.", {})],
], size=8.5, gap=0.245)
F.quote(s, 6.84, "세는 방법을 먼저 정하고, 세기 전까지는 추정치라고 적어 둔다.",
        x=2.25, w=10.35, size=9)
...
F.lines(s, 6.94, 6.46, 5.70, [
    ...
    "정식 과제 등록 후 17개 센터로 확산 — 추가 하드웨어 구매 없이",
], size=8.5, gap=0.215)
