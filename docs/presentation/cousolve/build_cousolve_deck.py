...
t = tables(s)[0]
ROWS = [
    "적재 파렛트가 작업자의 시야를 가리고, 장비 소음에 경적이 묻힌다. "
    "존과 로케이션 사이 교차 구간에서 PIT 와 보행자가 서로를 보지 못한 채 접근한다.",
    "접근을 사전에 감지해 사람에게 알린다. 현행 I-PAS 는 3m 안에서 장비를 자동 셧다운한다 (EOD).",
    "I-PAS 태그를 단 장비 · 인원만 덮인다. 보행자끼리 · EPJ · 미장착 장비 구간에는 "
    "감지도 알림도 없어, 위험 상황을 확인할 방법 자체가 없다.",
...
for i, v in enumerate(ROWS):
    F.cell(t.cell(i, 1), v, 9.5, PP_ALIGN.LEFT, space=1.25)
...
# ── 7. 2. Tradeoffs ─────────────────────────────────────────
s = prs.slides[6]
F.zone(s, 2.25, 2.26, 10.35, [
    (None, [
        [("넓게 울리면 꺼버린다", {"bold": True}), ("  —  민감도를 올릴수록 경보 피로가 커지고, "
          "꺼 둔 단말은 보호가 0 이 된다", {"color": F.INK2})],
        [("촘촘히 스캔하면 1교대를 못 버틴다", {"bold": True}), ("  —  감지 주기와 배터리 · 발열이 "
          "정면으로 맞선다", {"color": F.INK2})],
        [("빨리 고치면 되돌아온다", {"bold": True}), ("  —  3개월 70회 배포로 즉시 대응했지만, "
          "고친 증상이 다시 관측됐다", {"color": F.INK2})],
        [("앱은 장비를 세우지 못한다", {"bold": True}), ("  —  경보까지가 한계다. 3m 물리 정지는 "
          "대체 불가", {"color": F.INK2})],
        [("개인 폰에 임시 설치돼 있다", {"bold": True}), ("  —  지금 도는 것과 지속 가능한 것이 "
          "다르다", {"color": F.INK2})],
    ]),
], gap=0.315)
F.zone(s, 2.25, 4.18, 10.35, [
    (None, [
        [("자동 음소거", {"bold": True, "color": F.GREEN2}),
         ("  같은 기기 · 같은 등급에 5초 이상 머물면 소리 · 진동을 끈다. 표시 · 기록은 유지하고, "
          "등급이 오르면 즉시 재발령 (v1.1.61)", {})],
        [("세이프존", {"bold": True, "color": F.GREEN2}),
         ("  휴게실 · 충전 구역에 비콘을 두면 그 안에서는 경보를 억제한다. 존 안의 기기는 상대에게도 "
          "안전으로 보인다 (v1.1.62)", {})],
        [("이탈 재발령 억제", {"bold": True, "color": F.GREEN2}),
         ("  멀어진 뒤 경고 범위에서 다시 울리지 않게 했다 (v1.1.51)", {})],
        [("CI 회귀 게이트", {"bold": True, "color": F.GREEN2}),
         ("  고친 증상을 테스트로 고정한다. 골든 테스트가 실패하면 릴리스가 자동 차단된다 (Phase 1 완료)",
          {})],
...
F.zone(s, 2.25, 2.26, 10.35, [
    (None, [
        [("미발령 0건", {"bold": True, "color": F.GREEN2}),
         ("   경고 · 위험 범위에 들어왔는데 경보가 뜨지 않은 사례", {})],
        [("오발령 0건", {"bold": True, "color": F.GREEN2}),
         ("   정지 중 반복 경보 · 이탈 후 재발령", {})],
        [("감지 중단 0건", {"bold": True, "color": F.GREEN2}),
         ("   상시 알림이 ‘이상’ 으로 바뀌거나 서비스가 사라짐", {})],
        [("회귀 재발 0건", {"bold": True, "color": F.GREEN2}),
         ("   이전에 고친 증상이 다시 관측됨", {})],
        [("1교대 연속 구동", {"bold": True, "color": F.GREEN2}),
         ("   배터리 · 발열이 작업 시간을 버티는가", {})],
        [("서베이 종합 만족도", {"bold": True, "color": F.GREEN2}),
         ("   10점 척도, WF11 · WF21 · WF25 (회수 전)", {})],
    ]),
], gap=0.30)
F.zone(s, 2.25, 4.18, 10.35, [
    (None, [
        [("·  경보 이력 — Firebase 에 등급 · 거리 · 시각이 기록된다. 미발령 · 오발령은 이 기록과 "
          "현장 진술을 맞춰 판정한다.", {})],
        [("·  현장 서베이 — 10점 척도(10 매우 만족 ~ 1 매우 미흡). 센터별 응답자 수와 테스트 기간을 "
          "함께 적어야 표본 크기를 판단할 수 있다.", {})],
        [("·  실기 검증 — 단계당 확인 항목 1건. 전 단말 배포 전 1대에서 먼저 확인한다.", {})],
        [("·  CI 골든 테스트 — 안전 크리티컬 경로가 실패하면 릴리스가 자동 차단된다 (Phase 1 에서 동작 중).",
          {})],
        [("·  지표는 앱 동작 기준으로만 정의했다. ", {}),
